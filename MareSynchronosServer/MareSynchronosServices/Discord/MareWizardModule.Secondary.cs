...
public partial class MareWizardModule
{
    ...
    public async Task ComponentSecondary()
    {
        ...
        eb.WithTitle("辅助UID");
        eb.WithDescription("你可以在这里创建辅助UID " + Environment.NewLine + Environment.NewLine
            + "辅助UID拥有完全分离的配对列表，加入的同步贝，显示的UID等等。" + Environment.NewLine
            + "如果你想双开游戏，或者给小号一个私密的身份，请在此创建辅助UID。" + Environment.NewLine + Environment.NewLine
            + "__提示:__ 对于让小号使用Mare，创建辅助UID _不是_ 必须的。" + Environment.NewLine + Environment.NewLine
            + $"你目前拥有 {secondaryUids} 个辅助UID，最多可拥有的数量是20个。");
        ComponentBuilder cb = new();
        AddHome(cb);
        cb.WithButton("创建辅助UID", "wizard-secondary-create:" + primaryUID, ButtonStyle.Primary, emote: new Emoji("2️⃣"), disabled: secondaryUids >= 20);
    ...
    public async Task HandleAddSecondary(MareDbContext db, EmbedBuilder embed, string primaryUID)
    {
        ...
        embed.WithDescription("一个辅助UID创建完成，将以下的信息输入Mare的服务设置页面。");
